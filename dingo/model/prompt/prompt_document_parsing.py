...
class PromptDocumentParsingQuality(BasePrompt):
    ...
    *关键评估原则与指南*
    ...
    3.  **Markdown渲染** 应基于Markdown的渲染后内容进行评估，而不仅仅是原始源代码。例如，如果转义的特殊字符（如 "\~" 代表 "~"）能够正确渲染，则视为正确。
    ...
    12. **图像占位符评估**  Markdown 中的图像占位符（例如 ![]('img_url')）通常表示OCR系统识别到原始图像中存在图片。如果原始图像中相应位置确实存在图片，并且该占位符大致对应了图片的位置（例如，紧邻图注文本），则此占位符本身不应被视为错误。如果与图片关联的图注（如“图1”、“表1”）在 Markdown 中识别错误、丢失或格式错误，这应根据具体情况归类为相应的文本或格式错误（如“文本识别错误”、“文本内容识别遗漏”、“文本格式丢失”等），而不是图片占位符本身的错误。占位符本身仍然可以被认为是正确的，如果它指示了图片的存在。评估的重点是占位符是否准确反映了“此处有图”的信息，而不是占位符的具体 URL 或文件名内容。
    ...
    *输出格式*
    ...
    "errors": [
        ...
        "error_category": "OCR相关问题", // 错误的大类，如“公式相关问题”
