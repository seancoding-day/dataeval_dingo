...
@Model.rule_register("QUALITY_BAD_FLUENCY", ["pretrain"])
class RuleTextPerplexity(BaseRule):
    """Check whether text perplexity exceeds the configured threshold.

    Perplexity is calculated with a causal language model. Lower values
    indicate that the text is more fluent and predictable under the selected
    model. Because perplexity values are model-dependent, callers can override
    both ``dynamic_config.model`` and ``dynamic_config.threshold``.
    """
    ...
    def _calculate_perplexity(cls, content, tokenizer, model, stride):
        ...
        for begin in range(0, sequence_length, stride):
            ...
            # Causal language models shift labels by one token internally.
