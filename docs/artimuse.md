...
在仓库根目录运行 `python examples/artimuse/artimuse.py`（[examples/artimuse/artimuse.py](../examples/artimuse/artimuse.py)）可完成评估；如设置 `output_path`，将在该目录生成带时间戳与短 ID 的子目录，包含 `summary.json` 与逐条明细。
