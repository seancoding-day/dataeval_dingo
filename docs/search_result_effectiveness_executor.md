...
The HTML candidate pattern requires a valid tag name immediately after `<` or `</`. Navigation text such as `< Previous page | Next page >` is not treated as markup, while real academic formatting tags such as `<span>`, `<i>`, `<sub>`, and `<scp>` remain detectable.
