...
| `QUALITY_BAD_TIMELINESS` | RuleDataTimeRange | Checks whether `data.dt` falls within the configured `dt_start` and `dt_end` range required by the target application scenario. | High-quality dataset quality evaluation specification (SAC/TC609) | N/A | N/A |
